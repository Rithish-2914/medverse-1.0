...
export async function GET(req: NextRequest) {
  ...
  const rows = await sql`SELECT id, round_idx, content, file_url, file_name, status, decided_by, feedback, decided_at, created_at FROM submissions WHERE team_code = ${teamCode} AND round_idx = ${roundIdx} ORDER BY created_at ASC` as any[];
  
  const twistRows = await sql`SELECT text FROM twist_deck WHERE revealed = 1 ORDER BY id DESC LIMIT 1` as any[];
  const twistActive = twistRows[0]?.text === 'TWIST_PHASE_ACTIVE';
  
  const kit = await sql`SELECT disease FROM kits WHERE team_code = ${teamCode}` as any[];
  let docLink = '';
  ...
  return NextResponse.json({ submissions: rows, docLink, twist: specificTwist });
}
